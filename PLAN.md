...
passed, including an offline reload through Cloudflare. Pending: the owner's
install on a real Android phone (install, airplane mode, open). After that the
six build steps are done; next, on the owner's word: the naming conventions
session ("Decide before the next route is drawn"), then M8 onward.
